      ...
      for (let i = -15; i < 85; i++) {
        ...
        if (!this.state.items[strTime]) {
          ...
          //if (appointment != null || appointment !== undefined ) {
            firebase.firestore().collection("users").doc(initialEmail).collection("events").doc("appointment").collection("date").doc(date).collection("appointment").get().then(function(querySnapshot) {
              querySnapshot.forEach(function(doc) {
                  // doc.data() is never undefined for query doc snapshots
                  this.state.items[strTime].push({
                    //firebase.firestore().collection('users').get('events')
                    name: 'Appointment for ' + doc.data().name + '\nTime of appointment: ' + doc.date().time, // + Put the actual time of the event here
